...
export default function LoginPageWithMesa() {
  ...
  // Obtener id_mesa de los parámetros de la URL
  const mesaId = Array.isArray(params.id_mesa) ? params.id_mesa[0] : params.id_mesa || "";

  // Fetch del número de mesa cuando el componente se monta
  useEffect(() => {
    if (mesaId && mesaId !== 'sin-mesa') {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (isNombreValid && isEmailValid) {
      ...
      try {
        ...
        // Token ya está guardado en cookies por loginUser()
